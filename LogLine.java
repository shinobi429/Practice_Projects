public class LogLine {
    ...
    public LogLevel getLogLevel() {
        return LogLevel.getByName(line.substring(1, line.indexOf(":")-1));
    }
...
Next, implement the LogLine.getLogLevel() method that returns the parsed the log level of
a log line:
...
Unfortunately, occasionally some log lines have an unknown log level. To gracefully handle
these log lines, add an UNKNOWN element to the LogLevel enum which should be returned when
...
The log level of a log line is quite verbose. To reduce the disk space needed to store the
log lines, a short format is developed: 
