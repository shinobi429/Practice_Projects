...
0b0000_1011 >> 2;
// # => 0b0000_0010
The << operator always inserts 0s on the right hand side. However, >> inserts the same bit as the left most bit (1 if the number is negative or 0 if positive).
...
// # => 0b0010_0100
Bitwise OR
The bitwise OR (|) operator takes two values and performs an OR on each bit. It compares each bit from the first value with the bit in thes same position from the second value. If either bit is 1, the result's bit is 1. Otherwise, it is 0.
